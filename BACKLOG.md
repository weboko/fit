...
- [ ] **F30 — Unlock model-level SwiftData tests + import-integrity regression** (next up)
  The last genuinely-valuable hardening item: the app's CORE value is data
  integrity (capture + clean export), but the importer/merge/persistence layer is
  untested because per-test `ModelContainer`s crash the test host (the F16 issue:
  the host app's CloudKit container conflicts). Fix: make the test host use an
  in-memory container under XCTest (detect `XCTestConfigurationFilePath` in
  `FitApp`/`PersistenceController` and skip the CloudKit container), so a single
  per-test in-memory `ModelContainer` works. Then add the **F26 import regression
  test** (a re-import omitting `title`/`notes` must NOT blank an existing value)
  and an **exercise-merge test** (sets move to canonical, old name kept as alias).
  Medium risk (touches the test-host launch — watch CI; main stays safe behind the
  gate), high value (protects the app's most important behavior). If isolating the
  container proves to destabilize the test host, revert the host change and keep
  tests context-free — do NOT leave the test job flaky.
- _After F30 the backlog is genuinely empty of clearly-valuable, SPEC-aligned work
  (the only leftovers are SPEC "could-have-later" big rocks — Apple Watch app,
  voice notes — none worth the cost for a single user). Prefer pausing for new
  user direction over manufacturing noise._
